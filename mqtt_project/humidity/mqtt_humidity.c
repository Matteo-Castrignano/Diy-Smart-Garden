	...
	uint16_t chunk_len)
{
	if (strcmp(topic, "irrigation") == 0) {
		...
		if (strcmp((const char*)chunk, "ON") == 0) {
			printf("Start irrigation!\n");
			irrigation = true;
		}
		else if (strcmp((const char*)chunk, "OFF") == 0) {
			printf("Stop irrigation!\n");
			irrigation = false;
		}
		else {
	...
	while (1) {
		...
		if ((ev == PROCESS_EVENT_TIMER && data == &periodic_timer) ||
			...
			if (state == STATE_SUBSCRIBED) {
				...
				if (increase) {
					humidity = humidity + (rand() % 5 + 1);
					increase = false;
				}
				else {
					humidity = humidity - (rand() % 3 + 1);
					increase = true;
				}
				...
				sprintf(app_buffer, "{\"node\": %d, \"humidity_value\": %d}", node_id, humidity);
